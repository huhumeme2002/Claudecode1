...
router.post('/', verifyAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { key_id, key: keyString, duration_days } = req.body;

    if ((!key_id && !keyString) || !duration_days || duration_days <= 0) {
      res.status(400).json({ error: 'Missing or invalid key_id/key / duration_days' });
      return;
    }
